...
class PlanGraphLevel(object):
    ...
    def update_action_layer(self, previous_proposition_layer):
        ...
        for action in all_actions:
            if previous_proposition_layer.all_preconds_in_layer(action) \
                    and not mutex_in_prev_layer(action, previous_proposition_layer):
    ...
    def update_proposition_layer(self):
        ...
        for add_prop in props:
            prop = Proposition(add_prop)
            prop.set_producers(props[add_prop])
    ...
    def update_mutex_proposition(self):
        ...
        for p in current_layer_propositions:
            for q in current_layer_propositions:
                if p != q and mutex_propositions(p, q, current_layer_mutex_actions):
...
def mutex_propositions(prop1, prop2, mutex_actions_list):
    ...
    for a1 in prod1:
        for a2 in prod2:
            if not Pair(a1, a2) in mutex_actions_list:
                return False

    return True

def mutex_in_prev_layer(action, layer):
    pre1 = action.get_pre()
    pre2 = action.get_pre()

    for p in pre1:
        for q in pre2:
            if p != q and layer.is_mutex(p, q):
                return True
    return False